...
import { getAllBrand, deleteBrand, createBrand, editBrand } from '../../../../services/BrandService';
import { Modal, Input } from 'antd';
class ManagerBrand extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataBrand: [],
    ...
    handleBrand = async (input, id) => {
        try {
            let data = {}
            if (input == 'delete') { data = await deleteBrand(id, this.state.accessToken); }
            else {
                if (!this.state.dataHandle.name) { toast.error('Vui lòng điền đầy đủ thông tin'); return; }
                if (input == 'edit') { data = await editBrand(this.state.dataHandle, this.state.accessToken); }
                if (input == 'create') { data = await createBrand(this.state.dataHandle, this.state.accessToken); }
            }
            if (data && data.data && data.data.errCode == 0) {
    ...
    render() {
        ...
        return (
            <>
                    ...
                    <div className=' w-full pb-[20px] '>
                        <div className='bg-white w-full rounded-[10px] shadow-md'>
                            <div className='grid grid-cols-8 text-center font-[500] py-[5px] bg-[#343a40] text-white rounded-t-[10px]'>
                                ...
                                <div className='col-span-4'><span>THƯƠNG HIỆU</span></div>
                                ...
                                return (
                                    <div key={item.id} className='grid grid-cols-8 py-[4px] text-center items-center border-b-[1px] font-[450]'>
                                        ...
                                        <div className='col-span-3 text-white flex items-center justify-center space-x-[10px]'>
                                            ...
                                            <button className='bg-[#343a40] p-[4px] rounded-[3px] hover:bg-[#eb0000]'
                                                onClick={() => this.handleBrand('delete', item.id)}><AiOutlineDelete /></button>
                                        </div>
                                    </div>
                                )
                            })}
                ...
                <Modal width={300}
                    onCancel={() => this.onClickCloseModal()} onOk={() => this.handleBrand('create')}
                    title="Tạo mới" open={this.state.isActive == 'create' ? true : false}>
                    <Input placeholder="Tên thương hiệu" onChange={(event) => this.handleOnChangeInput(event, 'name')} />
                </Modal>
                <Modal width={300}
                    onCancel={() => this.onClickCloseModal()} onOk={() => this.handleBrand('edit')}
                    title="Chỉnh sửa" open={this.state.isActive == 'edit' ? true : false}>
                    <Input value={this.state.dataHandle.name} placeholder="Tên thương hiệu" onChange={(event) => this.handleOnChangeInput(event, 'name')} />
                </Modal>
            </>
        );
    }
