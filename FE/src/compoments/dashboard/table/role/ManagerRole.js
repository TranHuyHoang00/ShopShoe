...
import { getAllRole, deleteRole, createRole, editRole } from '../../../../services/RoleService';
import { Modal, Input, } from 'antd';
class ManagerRole extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataRole: [],
    ...
    handleRole = async (input, id) => {
        try {
            let data = {}
            if (input == 'delete') { data = await deleteRole(id, this.state.accessToken); }
            else {
                if (!this.state.dataHandle.name || !this.state.dataHandle.type) { toast.error('Vui lòng điền đầy đủ thông tin'); return; }
                if (input == 'edit') { data = await editRole(this.state.dataHandle, this.state.accessToken); }
                if (input == 'create') { data = await createRole(this.state.dataHandle, this.state.accessToken); }
            }
            if (data && data.data && data.data.errCode == 0) {
    ...
    render() {
        ...
        return (
            <>
                    ...
                    <div className=' w-full pb-[20px] '>
                        <div className='bg-white w-full rounded-[10px] shadow-md'>
                            <div className='grid grid-cols-10 text-center font-[500] py-[5px] bg-[#343a40] text-white rounded-t-[10px]'>
                                ...
                                <div className='col-span-3'><span>TÊN QUYỀN</span></div>
                                <div className='col-span-3'><span>LOẠI</span></div>
                            ...
                            {dataRole && dataRole.map((item, index) => {
                                return (
                                    <div key={item.id} className='grid grid-cols-10 py-[4px] text-center items-center border-b-[1px] font-[450]'>
                                        ...
                                        <div className='col-span-3'><span>{item.type == 1 ? 'Nhân viên' : 'Khách hàng'}</span></div>
                                        <div className='col-span-3 text-white flex items-center justify-center space-x-[10px]'>
                                            ...
                                            <button className='bg-[#343a40] p-[4px] rounded-[3px] hover:bg-[#eb0000]'
                                                onClick={() => this.handleRole('delete', item.id)}><AiOutlineDelete /></button>
                                        </div>
                                    </div>
                                )
                            })}
                ...
                <Modal width={300}
                    onCancel={() => this.onClickCloseModal()} onOk={() => this.handleRole('create')}
                    title="Tạo mới" open={this.state.isActive == 'create' ? true : false}>
                    <div className='space-y-[5px]'>
                        <Input placeholder="Tên quyền" onChange={(event) => this.handleOnChangeInput(event, 'name')} />
                ...
                <Modal width={300}
                    onCancel={() => this.onClickCloseModal()} onOk={() => this.handleRole('edit')}
                    title="Chỉnh sửa" open={this.state.isActive == 'edit' ? true : false}>
                    <div className='space-y-[5px]'>
                        <Input value={this.state.dataHandle.name} placeholder="Tên quyền" onChange={(event) => this.handleOnChangeInput(event, 'name')} />
                        <select value={this.state.dataHandle.type} onChange={(event) => this.handleOnChangeInput(event, 'type')}
                            className='w-full border h-[35px] rounded-[5px]'>
                            <option>Loại</option>
                            <option value={1}>Nhân viên</option>
                            <option value={2}>Khách hàng</option>
